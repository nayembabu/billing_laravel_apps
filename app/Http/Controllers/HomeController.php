...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $payment_recieved_index = DB::table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('payments.amount');
            $payment_sent_index = DB::table('payments')->whereNotNull('purchase_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('payments.amount');
            $return_index = Returns::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
    ...
    public function dashboardFilter($start_date, $end_date)
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $total_purchase = Purchase::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date)->sum('grand_total');

            $total_expense = Expense::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->whereDate('created_at', '<=' , $end_date)->sum('amount');
            ...
            $cash_in_hand = $payment_recieved_index - $payment_sent_index - $return_index[0]->grand_total + $purchase_return_index[0]->grand_total - $expense_index - $payroll_index;

            $data[0] = $revenue;
            $data[1] = $return;
            $data[2] = $profit;
            $data[3] = $purchase_return;
            $data[4] = $total_purchase;
            $data[5] = $total_expense;
            $data[6] = $cash_in_hand;
            ...
            $revenue -= $return;
            $profit = $revenue + $purchase_return - $product_cost;
            $total_purchase = Purchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');

            $total_expense = Expense::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('amount');

...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $c = Currency::where('id', $sale->currency_id)->get();
                foreach($c as $a){
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            $lims_default_currency = GeneralSetting::findOrFail(1);
            $getCurrencyExchangeRate = Currency::where('id', $lims_default_currency->currency)->pluck('exchange_rate')->first();
            return view('sale.create', compact('lims_warehouse_list', 'lims_supplier_list', 'lims_unit_list', 'lims_category_list', 'getCurrencyExchangeRate', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data', 'lims_currency_all', 'lims_default_currency'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->sale_type == 'add_sale'){
            $data['currency_id'] = $request->currency_id;
        }else{
            $cd = GeneralSetting::latest()->first();
            $data['currency_id'] = $cd->currency;
        }
        ...
        $qty = $data['qty'];
        $temp_prod_name = $data['temp_product_name'];
        ...
        foreach ($product_id as $i => $id) {
            ...
            $lims_product_data->update([
                'temp_product_name' => $temp_prod_name[$i]
            ]);
            ...
            $product_sale['created_at'] = $data['created_at'];
            $product_sale['net_weight'] = $data['net_weight'][$i];
            $product_sale['gross_weight'] = $data['gross_weight'][$i];
    ...
    public function productSaleData($id)
    {
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
            ...
            $product_sale[10][$key] = $product->temp_product_name;
            $product_sale[11][$key] = $product->code;
            ...
            $product_sale[9] = $getCurrency->code;
    ...
    public function instantProductgencode()
    {
        $id = Keygen::numeric(8)->generate();
        return $id;
    }

    public function instantProductAddPurchase(Request $request)
    {
        try {
            ...
            $purchase_id = Purchase::insertGetId([
                ...
                'total_discount'    => 0,
                'total_tax'         => 0,
                'total_cost'        => $request->qty * $request->cost,
                'grand_total'       => $request->qty * $request->cost,
                'paid_amount'       => 0,
            ...
            ProductPurchase::create([
                ...
                'qty'               => $request->qty,
                'recieved'          => $request->qty,
                'purchase_unit_id'  => 1,
                'net_unit_cost'     => $request->cost,
                'discount'          => $request->order_discount,
                'tax'               => 0,
                'tax_rate'          => $request->order_tax_rate,
            ...
            return back()->with('success', 'Product add & purchase successfully!');
        } catch (\Exception $e) {
    ...
    {
        $lims_sale_data = Sale::find($id);
        $getCurrency = Currency::where('id', $lims_sale_data->currency_id)->first();
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        ...
        $numberInWords = $numberTransformer->toWords($lims_sale_data->grand_total * $getCurrency->exchange_rate);

        return view('sale.invoice', compact('getCurrency', 'lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }

...
class ProductController extends Controller
{
    ...
    {
        return ProductVariant::join('variants', 'product_variants.variant_id', '=', 'variants.id')->select('variants.name', 'product_variants.item_code', 'product_variants.additional_cost', 'product_variants.additional_price', 'product_variants.qty')->orderBy('product_variants.position')->where('product_id', $id)->get();
        //return json_decode(Product::select('variant_data')->find($id)->variant_data);
    }
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            return view('product.edit',compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function importProduct(Request $request)
    {
        ...
        while($columns=fgetcsv($file))
        {
           ...
           $product->pack_size = $data['packsize'];
           $product->net_weight = $data['netweight'];
           $product->gross_weight = $data['grossweight'];
           $product->expired_at = date('Y-m-d', strtotime($data['expiredat']));

...
<body @if ($theme == 'dark') class="dark-mode dripicons-brightness-low" @else class="" @endif
    ...
    <nav class="side-navbar">
        ...
        <ul id="side-main-menu" class="side-menu list-unstyled">
            ...
            @if ($index_permission_active)
                <li><a href="#purchase" aria-expanded="false" data-toggle="collapse"> <i
                    ...
                    <ul id="purchase" class="collapse list-unstyled ">
                        ...
                        $add_permission_active = DB::table('permissions')
                            ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                            ->where([['permissions.name', 'purchases-add'], ['role_id', $role->id]])
                            ->first();
                        ?>
                        @if ($add_permission_active)
            ...
            ?>
            @if ($index_permission_active)
            <li><a href="#customer-list" aria-expanded="false" data-toggle="collapse"> <i
                        class="dripicons-user-group"></i><span>{{ trans('Customer List') }}</span></a>
                <ul id="customer-list" class="collapse list-unstyled {{ Route::currentRouteName() == 'customer.index' ? 'show' : '' }}">
                    <li id="exp-cat-menu" class="{{ Route::currentRouteName() == 'customer.index' ? 'active' : '' }}"><a
                            href="{{ route('customer.index') }}">{{ trans('Customer List') }}</a>
                    </li>
                </ul>
            </li>
            @endif
    ...
    <div class="page">
        ...
        <footer class="main-footer">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-12">
                        <p>&copy; {{ $general_setting->site_title }} | Managed
                            {{ trans('file.By') }} <span
                                class="external">{{ $general_setting->developed_by }}</span> | Large iT-V 3.7</p>
                    </div>
                </div>
            </div>
        </footer>

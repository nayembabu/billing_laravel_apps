        ...
        <table class="table-data">
            <tbody>
                ...
                <tr>
                    <td colspan="2">
                        {!!$product_name!!}
                        <br>{{$product_sale_data->qty}} x {{number_format((float)(($product_sale_data->total * $getCurrency->exchange_rate) / $product_sale_data->qty), 2, '.', '')}}

                        @if($product_sale_data->tax_rate)
                            <?php $total_product_tax += $product_sale_data->tax ?>
                            [{{trans('file.Tax')}} ({{$product_sale_data->tax_rate}}%): {{$product_sale_data->tax * $getCurrency->exchange_rate}}]
                        @endif
                    </td>
                    <td style="text-align:right;vertical-align:bottom">{{number_format((float)($product_sale_data->total  * $getCurrency->exchange_rate), 2, '.', '')}}</td>
                </tr>
                @endforeach
            ...
            <!-- <tfoot> -->
                <tr>
                    ...
                    <th style="text-align:right">{{number_format((float)($lims_sale_data->total_price  * $getCurrency->exchange_rate), 2, '.', '')}}</th>
                </tr>
                ...
                <tr>
                    ...
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->order_tax *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->order_discount)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Order Discount')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->order_discount *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->coupon_discount)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Coupon Discount')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->coupon_discount *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->shipping_cost)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Shipping Cost')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->shipping_cost *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->pickup_cost)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Pickup Cost')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->pickup_cost *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->delivery_cost)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Delivery Cost')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->delivery_cost *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->packaging_cost)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Packaging Cost')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->packaging_cost *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                @if($lims_sale_data->bd_charges_cost)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.BD Charges')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->bd_charges_cost *  $getCurrency->exchange_rate, 2, '.', '')}}</th>
                </tr>
                @endif
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.grand total')}}</th>
                    <th style="text-align:right">{{number_format((float)($lims_sale_data->grand_total * $getCurrency->exchange_rate), 2, '.', '')}}</th>
                </tr>
                <tr>
                    @if($general_setting->currency_position == 'prefix')
                    <th class="centered" colspan="3">{{trans('file.In Words')}}: <span>{{$getCurrency->code}}</span> <span>{{str_replace("-"," ",$numberInWords)}}</span></th>
                    @else
                    <th class="centered" colspan="3">{{trans('file.In Words')}}: <span>{{str_replace("-"," ",$numberInWords)}}</span> <span>{{$getCurrency->code}}</span></th>
                    @endif
                </tr>
            </tbody>
            <!-- </tfoot> -->
